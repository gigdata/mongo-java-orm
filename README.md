...
FROM
   ...
   [WHERE 
     condition [, condition ...]]
   action
collection_name is the name of the collection that is being operated on.
condition is a query condition that must be met
action is the action to perform on the documents in the collection specified that match the specified conditions (or all documents if no conditions are specified)
Variable literals
...
elemMatch	performs an $elemMatch
mod	performs an $mod
...
nor	performs a $nor
and	performs an $and
predicate	enables the use of a javascript for a $where statement
FROM
   collection_name
   WHERE
...
inc	inc fieldName 1	Increments the given field name by the given number, negative numbers decrement
set	set fieldName 'newValue'	Gives the field a new value
unset	unset fieldName	Unsets (removes) the given field
push	push fieldName 'underwear'	Adds a value to a given field
push all	push all fieldName ['val1', 2, 'val3']	Adds values to a given field
add to set	add to set fieldName 'value'	Adds a value to a given field if the value doesn't already exist
add to set each	add to set fieldName each ['val', 'another', 3]	Adds each value to the given field if they don't already exist
pop	pop fieldName	Removes the last element in an array field
shift	shift fieldName	Removes the first element in an array field
pull	pull fieldName 'aValue'	Removes all occurrences of the value from the given array field
pull all	pull all fieldName ['a val', 'another', 10]	Removes each occurrence of the values from the given array field
rename	rename fieldName newName	Renames an existing field
bitwise or	bitwise or fieldName 210	Applys a bitwise OR to the given field name using the given integer
bitwise and	bitwise and fieldName 210	Applys a bitwise AND to the given field name using the given integer
hints
